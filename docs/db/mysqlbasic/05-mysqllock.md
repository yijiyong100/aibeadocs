...
　　后来看到一篇[文章](http://www.cnblogs.com/chenqionghe/p/4845693.html)，“防止更新丢失，并不能单靠数据库事务控制器来解决，需要应用程序对要更新的数据加必要的锁来解决”。瞬间，世界观都崩塌了。非常不敢相信，于是自己写了代码检验一下。

　　数据库表是这样的。用count字段来做100次累加。

<img class= "zoom-custom-imgs" :src="$withBase('/assets/img/db/mysqlbasic/lock-1.png')" alt="wxmp">

　　为了保证实验的科学性，先确认了数据库是InnoDB的，这样才有事务机制；也确认了隔离性级别

<img class= "zoom-custom-imgs" :src="$withBase('/assets/img/db/mysqlbasic/lock-2.png')" alt="wxmp">
...
<img class= "zoom-custom-imgs" :src="$withBase('/assets/img/db/mysqlbasic/lock-3.png')" alt="wxmp">
...
　　为了找到问题的根源，为了拯救我崩溃的世界观，我又去回顾了数据库事务的知识。借鉴 [这篇](http://blog.csdn.net/gklifg/article/details/38752691)
...
　　mysql用意向锁（另一种机制）来解决事务并发问题，为了区别封锁协议，弄了一个新概念隔离性级别：包括Read Uncommitted、Read Committed、Repeatable Read、Serializable，见[这篇](http://blog.csdn.net/gklifg/article/details/38752691)。mysql 一般默认Repeatable Read。

<img class= "zoom-custom-imgs" :src="$withBase('/assets/img/db/mysqlbasic/lock-4.png')" alt="wxmp">

<img class= "zoom-custom-imgs" :src="$withBase('/assets/img/db/mysqlbasic/lock-5.png')" alt="wxmp">
...
<img class= "zoom-custom-imgs" :src="$withBase('/assets/img/db/mysqlbasic/lock-6.png')" alt="wxmp">

　　b. 没有事务，不用考虑并发问题，世界和平~

　　c. 由于锁的粒度太大，所以当该表写并发量较高时，要等待的查询就会很多了。优化见 [这里](http://www.cnblogs.com/chenqionghe/p/4845693.html)。

### 2、InnoDB的行锁和表锁

　　没有特定的语法。mysql的行锁是通过索引体现的，[参考](http://blog.techbeta.me/2014/11/mysql-for-update/)。
...
<img class= "zoom-custom-imgs" :src="$withBase('/assets/img/db/mysqlbasic/lock-7.png')" alt="wxmp">
...
<img class= "zoom-custom-imgs" :src="$withBase('/assets/img/db/mysqlbasic/lock-8.png')" alt="wxmp">
...
<img class= "zoom-custom-imgs" :src="$withBase('/assets/img/db/mysqlbasic/lock-9.png')" alt="wxmp">
...
　　乐观锁也没有指定怎么检测并发冲突，下面是常见的两种做法（[参考](http://chenzhou123520.iteye.com/blog/1863407)）：
...
<img class= "zoom-custom-imgs" :src="$withBase('/assets/img/db/mysqlbasic/lock-10.png')" alt="wxmp">
...
## 参考文章
* https://www.cnblogs.com/liliuguang/p/13954470.html