...
转载自:http://blog.51cto.com/wn2100/2049534
...
### 8. 创建GBK字符集的数据库danny，并查看已建库完整语句

mysql> create database T2 default character set utf8;
...
### 9. 创建用户danny，使之可以管理数据库danny

mysql> grant select,update,insert,delete,alter on danny.* to danny@'localhost' identified by '123456';

 

### 10. 查看创建的用户danny拥有哪些权限
...
mysql> insert into test (id,name) values (1,'danny');

 

### 16. 再批量插入2行数据 “2,测试”，“3,dannyedu”

mysql> insert into test (id,name) values (2,'测试'),(3,'dannyedu');

 

### 17. 查询名字为danny的记录

mysql> select * from test where name='danny';

 

### 18. 把数据id等于1的名字danny更改为oldgirl
...
### 20. 不退出数据库,完成备份danny数据库

mysql> system mysqldump -uroot -p123456 -B -x -F --events danny >/opt/bak.sql
...
### 22. 删除表test和danny数据库并查看

mysql> drop table test;

mysql> drop database danny;
...
mysql> alter database danny default character set utf8;

mysql> alter table test default character set utf8;

或

mysql> alter database danny charset utf8;
...
mysql> alter table test add index index_name_phone(name(6),phone(8))

 

### 32. 查询手机号以135开头的，名字为danny的记录（提前插入）。

mysql> select * from test where name='danny' and shouji like "135%";

 　　　

### 33. 查询上述语句的执行计划（是否使用联合索引等）。

mysql> explain select * from test where name="danny" and shouji like "135%"\G
...
### 35. 收回danny用户的select权限。

mysql> revoke select on danny.* from danny@'localhost';
...
配置文件里注销--skip-grant-tables，再重启Mysql
...
### 46. 如何授权danny用户从172.16.1.0/24访问数据库。

mysql> grant all on *.* to danny@'172.16.1.%' identified by '123456';
...
<img class= "zoom-custom-imgs" :src="$withBase('/assets/img/mysqlop/basic/commknowledge-1.png')" alt="wxmp">
